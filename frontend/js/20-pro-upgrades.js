...
(function () {
  ...
  function injectShell() {
    ...
    // BUGFIX: antes esto hacía (qs('main')||document.body).prepend(shell),
    // metiendo el panel como PRIMER HIJO adentro de <main> — que ya es un
    // flex-row de 2 columnas (aside + content) con overflow:hidden y altura
    // calculada por flex. Un tercer bloque ahí adentro rompía ese layout en
    // vez de quedar como una franja arriba de todo. Ahora se inserta como
    // hermano, ANTES de <main> (después del <header>), a todo el ancho.
    if (anchor) anchor.before(shell);
  ...
  async function openAI() {
    ...
    el('proAiRun').addEventListener('click', async () => {
      ...
      try {
        ...
        if (typeof window.setAiContext === 'function') window.setAiContext(data.analysis || null);
        else {
          window.lastAnalysisData = data.analysis || null;
  ...
  function boot() {
    ...
    improveHistory();
    window.addEventListener("analysis-updated", (event) => {
